...
# Load the CSV file
file_name = "input_output_wasb.csv"
...
plt.plot(data['time_seconds'], data['x'], label='x-coordinate', marker='o')
plt.plot(data['time_seconds'], data['y'], label='y-coordinate', marker='o')
...
if end_frame is not None:
    ...
    plt.scatter(data['time_seconds'].iloc[end_frame], data['x'].iloc[end_frame], color='red', label='End Point (X)', zorder=5)
    plt.scatter(data['time_seconds'].iloc[end_frame], data['y'].iloc[end_frame], color='orange', label='End Point (Y)', zorder=5)
